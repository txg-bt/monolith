...
const ownsRestaurant = require("../utils/ownsRestaurant");
...
router.get("/restaurant/:restaurantId", authorization, async (req, res) => {
  try {
    ...
    if (!ownsRestaurant(restaurantId, user_id)) {
      logWritter("Not allowed to see reservations");

      return res.status(403).send("Not allowed to see reservations");
    }
    ...
    // get restaurants name based on ids from reservations and decorate data with that
    const restaurantIds = reservations.rows.map(
      (reservation) => reservation.restaurant_id
    );

    const restaurants = await pool.query(
      "SELECT * FROM restaurants WHERE restaurant_id = ANY($1)",
      [restaurantIds]
    );

    const decoratedReservations = reservations.rows.map((reservation) => {
      const restaurant = restaurants.rows.find(
